...
# basin_lc = read_csv('data/major_basin_summary.csv')


## plot ####

mines_all <- st_read('data/skytruth/med_0-10_thresh_ridgeMasked_1985-2024_firstMined.geojson')
...
# build a named list mapping basin name -> basin polygon (for clipping mines)
basin_polys <- list(
  Elk      = elk_basin,
  Kootenai = kootenai_basin,
  Kootenay = kootenay_basin
)

## loop over basins ####

for (basin_name in names(basin_polys)) {

  message('\n=== Processing basin: ', basin_name, ' ===')

  basin_poly <- basin_polys[[basin_name]]

  # --- prepare land cover time series (Panel 1) ---
  ...
  mines <- mines_all %>% st_transform(st_crs(basin_poly))
  mines <- st_intersection(mines, basin_poly %>% summarise(geometry = st_union(geometry)) %>% st_make_valid())
  ...
  if (nrow(mines) > 0) {
    ...
    mine_years   <- integer(0)
    mine_area_ts <- tibble(year = integer(0), area_km2 = numeric(0))
  }

  # --- prepare barren-mine overlap time series (Panel 3) ---

  basin_4326 <- basin_poly %>% st_transform(4326)
  basin_ext  <- ext(st_bbox(basin_4326)) + 0.01
  ...
  for (yr in all_years) {
    ...
    for (i in seq_len(nrow(tile_combos))) {
      ...
      if (is.na(f) || !file.exists(f)) next
      r <- rast(f, lyrs = band)
      ov <- intersect(ext(r), basin_ext)
      if (!is.null(ov)) rasters <- c(rasters, list(crop(r, basin_ext)))
    }
    ...
    # mask to basin
    basin_vect <- vect(basin_4326)
    yr_basin   <- mask(crop(yr_rcl, basin_vect), basin_vect)

    # total barren/snow/ice area (class 8)
    barren_mask <- yr_basin == 8
    barren_area <- global(barren_mask * cellSize(barren_mask, unit = 'km'),
                          'sum', na.rm = TRUE)[[1]]

    # cumulative mine extent up to this year
    ...
    if (length(valid_mine_yrs) == 0) {
      ...
      rm(yr_rast, yr_rcl, yr_basin, barren_mask); gc(verbose = FALSE)
      next
    }
    ...
    mine_rast <- rasterize(mine_vect, yr_basin, field = 1, background = 0)
    ...
    rm(yr_rast, yr_rcl, yr_basin, barren_mask, mine_rast, overlap_mask)
    gc(verbose = FALSE)
  }
  ...
  p1 <- ggplot(cur_lc, aes(x = year, y = pct, fill = class)) +
    geom_col(width = 1) +
    scale_fill_manual(values = lc_colors, name = 'Land cover') +
    labs(x = NULL, y = '% of basin',
         title = paste0(basin_name, ' Basin land cover composition')) +
  ...
  p2 <- ggplot(cur_lc_delta, aes(x = year, y = delta, color = class)) +
  ...
  combined <- (p1 / p2 / p3 / p4) +
    plot_annotation(
      title = paste0(basin_name, ' River Basin \u2014 Land Cover & Mining'),
      theme = theme(plot.title = element_text(face = 'bold', size = 14, hjust = 0.5))
    ) +
    plot_layout(heights = c(3, 3, 2, 2))

  out_png <- paste0('figs/', tolower(basin_name), '_landcover_mining.png')
